...
different policy, they support the same configuration options
 ...
 - **weekend-only**:
     default false, whether to turn the resource off only on the weekend
 - **default_tz**: which timezone to utilize when evaluating time
 - **tag**:
     default maid_offhours, which resource tag key to look for the resource's
     schedule.
 - **opt-out**:
     applies the default schedule to resource which do not specify any value.
     a value of `off` to disable/exclude the resource.
